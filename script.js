...
/* ==================== MODE: VIDEO ==================== */
const VIDEO_MAX_SECONDS = 20; // dijaga singkat biar ukuran file tidak kebesaran buat diupload
...
// Resolusi rekaman sekarang ngikutin resolusi kamera ASLI (bukan dipatok
// 480x854 lagi), supaya hasil video kenangan setajam video biasa. Tetap
// dikasih batas atas (MAX_REC_DIMENSION) buat 2 alasan:
// 1) Beauty butuh olah tiap frame — kalau dibiarkan sampai resolusi HP
//    flagship (mis. 4K), sebagian HP tamu bisa patah-patah/nge-lag.
// 2) File hasil rekam dikirim ke Worker dalam bentuk base64, yang server-nya
//    (worker.js) menolak kalau kepanjangan (lihat batas di sana) — resolusi
//    kegedean + durasi 20 detik bisa gagal upload.
// 1080px di sisi terpanjang sudah setara Full HD (lebih dari cukup buat video
// pendek di layar HP), jadi dipilih sebagai batas amannya.
const MAX_REC_DIMENSION = 1080;
...
function updateRecCanvasSize(){
  ...
  if(vw && vh){
    const scale = Math.min(1, MAX_REC_DIMENSION / Math.max(vw, vh));
    // dibulatkan ke angka genap — sebagian encoder video rewel kalau ukurannya ganjil
    REC_W = Math.round((vw * scale) / 2) * 2;
    REC_H = Math.round((vh * scale) / 2) * 2;
  }
  videoRecCanvas.width = REC_W; videoRecCanvas.height = REC_H;
...
function renderVideoFrame(){
  ...
  if(state.videoFilter === 'beauty'){
    // (1) Gambar video ASLI dulu penuh di canvas utama — ini jadi "dasar"
    //     yang tetap tajam (mata, alis, rambut, bibir tetap kelihatan jelas).
    drawVideoCover(videoRecCtx, videoRecPreview, REC_W, REC_H, mirror);
    // (2) Baru versi super-kecil->diperbesar (lembut/blur) ditumpuk TIPIS di
    //     atasnya dengan opacity rendah. Ini kuncinya biar hasilnya "mulus
    //     natural" bukan "mulus plastik" — detail wajah asli tetap tembus
    //     dari bawah, cuma noda/pori-pori/kerutan halus yang ke-samarin.
    drawVideoCover(_beautySmallCtx, videoRecPreview, BEAUTY_SMALL_W, BEAUTY_SMALL_H, mirror);
    videoRecCtx.save();
    videoRecCtx.globalAlpha = 0.45; // di bawah 0.5 supaya nggak flat/lebay
    ...
    videoRecCtx.restore();
    // (3) Cahaya hangat SANGAT tipis, cuma buat kasih kesan "glow" natural,
    //     bukan buat ubah warna kulit.
    videoRecCtx.save();
    videoRecCtx.globalCompositeOperation = 'soft-light';
    videoRecCtx.fillStyle = 'rgba(255,214,190,0.08)';
...
function startVideoRecording(activeBtn){
  ...
  try{
    videoRecorder = mimeType ? new MediaRecorder(recordStream, { mimeType }) : new MediaRecorder(recordStream);
  }catch(e){
