...
/* ==================== MODAL: DETAIL FOTO GALERI ====================
   Tahap ini baru TAMPILAN. Untuk item yang belum punya data VN/nama
   asli (mis. dari galeri GitHub), field-nya diisi data contoh (dummy)
   supaya desainnya kelihatan utuh — nanti disambungkan ke data
   sungguhan di tahap fungsi (upload VN ke server, dsb). */
...
function openPhotoModal(item){
  ...
  document.getElementById('modalDate').textContent = item.date || '25 AGUSTUS 2026';
  document.getElementById('modalTime').textContent = item.time || '10.57 WIB';
  document.getElementById('modalDur').textContent = item.audioDurationLabel || '00:15';
...
document.getElementById('finalShareBtn').addEventListener('click', async ()=>{
  ...
  const dataUrl = canvas.toDataURL('image/jpeg',0.9);
  const filename = uniqueFilename('wedding');
  state.gallery.unshift({ img: dataUrl, name: state.name, hasAudio: !!state.audioUrl, audioUrl: state.audioUrl || null });
  downloadCanvas(canvas, filename);
  if(state.audioUrl){
