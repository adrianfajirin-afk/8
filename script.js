...
document.getElementById('shareOnlyBtn').addEventListener('click', async ()=>{
  ...
  // PENTING: download foto ke HP tamu dilakukan SETELAH upload selesai,
  // bukan sebelumnya. Di Safari iPhone, klik <a download> untuk file
  // data: (foto) suka membuat Safari benar-benar berpindah halaman
  // sesaat (muncul dialog "Apakah Anda ingin mengunduh...") — ini
  // memutus koneksi upload yang sedang berjalan kalau dilakukan sebelum
  // fetch() selesai. Makanya urutan download dipindah ke paling akhir.
...
document.getElementById('finalShareBtn').addEventListener('click', async ()=>{
  ...
  // setelah itu foto & audio di-download ke HP tamu. Di iPhone/Safari,
  // meng-klik <a download> untuk file data:/blob: (baik foto maupun
  // audio) sebelum fetch selesai bisa membuat Safari benar-benar
  // berpindah halaman sesaat (muncul dialog "Apakah Anda ingin
  // mengunduh...") — ini menginterupsi koneksi upload yang sedang
  ...
  // Download foto & audio ke HP tamu dilakukan paling akhir, setelah
  // semua proses upload selesai (berhasil ataupun gagal), supaya tidak
