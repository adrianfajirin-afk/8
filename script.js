...
async function fetchGitHubGallery(){
  ...
  try{
    ...
    if(res.ok){
      ...
      const list = mapManifestList(JSON.parse(jsonText));
      if(list) return list;
    } else if(res.status === 404){
      return null; // belum ada momen sama sekali -> wajar untuk situs yang baru mulai dipakai
    } else if(res.status !== 403){
      console.error('[GitHub] Gagal ambil manifest:', res.status);
      showDebugBanner(`[GitHub] Gagal ambil manifest: status ${res.status}`);
    }
    // status 403 (kena limit akses tanpa token) atau respons tak terduga -> coba jalur cadangan
...
document.getElementById('finalShareBtn').addEventListener('click', async ()=>{
  ...
  // PENTING: proses upload (fetch) dijalankan DULU sampai selesai, baru
  // setelah itu file audio di-download ke HP tamu. Di iPhone/Safari,
  // meng-klik <a download> untuk file blob: audio sebelum fetch selesai
  // bisa membuat Safari menginterupsi koneksi upload yang sedang
  // berjalan (muncul sebagai error "Load failed"). Dengan urutan ini,
  // upload tidak lagi terganggu oleh proses download lokal.
  const imageUrl = await uploadToGitHub(dataUrl, filename);
  if(imageUrl){
    console.log('[GitHub] Foto tersimpan:', imageUrl);
  ...
  // Download audio ke HP tamu dilakukan paling akhir, setelah semua
  // proses upload selesai (berhasil ataupun gagal), supaya tidak
  // mengganggu jalannya fetch() di atas.
  if(state.audioUrl){
