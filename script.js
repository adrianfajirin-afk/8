...
async function startVideoCamera(){
  ...
  try{
    ...
    videoRecPreview.style.setProperty('--mirror', state.videoFacingMode === 'user' ? -1 : 1);
    try{ await videoRecPreview.play(); }catch(e){}
    videoRecHint.textContent = 'Izinkan akses kamera & mikrofon. Maks. ' + VIDEO_MAX_SECONDS + ' detik.';
  }catch(err){
...
videoRecordBtn.addEventListener('click', ()=>{
  ...
  if(!videoRecording) startVideoRecording(); else stopVideoRecording();
});
...
function startVideoRecording(){
  ...
  try{
    videoRecorder = mimeType ? new MediaRecorder(videoStream, { mimeType }) : new MediaRecorder(videoStream);
  }catch(e){
