...
// Canvas kecil dipakai buat trik "downscale lalu upscale" — cara murah buat
// hasil kulit lebih halus tanpa perlu olah piksel satu-satu (yang berat).
const _beautySmall = document.createElement('canvas');
let BEAUTY_SMALL_W = Math.round(REC_W/6), BEAUTY_SMALL_H = Math.round(REC_H/6);
_beautySmall.width = BEAUTY_SMALL_W; _beautySmall.height = BEAUTY_SMALL_H;
const _beautySmallCtx = _beautySmall.getContext('2d');
...
function updateRecCanvasSize(){
  ...
  videoRecCanvas.width = REC_W; videoRecCanvas.height = REC_H;
  BEAUTY_SMALL_W = Math.max(40, Math.round(REC_W/6));
  BEAUTY_SMALL_H = Math.max(40, Math.round(REC_H/6));
  _beautySmall.width = BEAUTY_SMALL_W; _beautySmall.height = BEAUTY_SMALL_H;
}
...
/* ==================== RESEP 6 FILTER VIDEO ====================
   Semua filter dibangun dari 3 "bahan" yang sama, cuma beda takaran:
   1) skinSmooth — tumpuk versi super-kecil->diperbesar (blur murah) di atas
      video asli dengan opacity rendah, bikin kulit lebih halus tanpa nge-
      olah piksel satu-satu (berat kalau dilakukan tiap frame).
   2) glow       — sama seperti skinSmooth tapi pakai blend 'screen' supaya
      highlight-nya "mekar"/bloom, dipakai buat kesan dreamy/soft.
   3) tint       — sapuan warna tipis (blend mode tertentu) buat ubah "mood"
      warna keseluruhan (hangat/gading/dingin dst).
   4) brighten   — sapuan putih tipis blend 'screen' buat naikin kecerahan.
   Sengaja TIDAK pakai ctx.filter (CSS filter di canvas) karena dukungannya
   tidak konsisten di Safari/iOS — semua efek di sini murni drawImage +
   composite blend supaya konsisten di semua browser & ikut kerekam. */
const VIDEO_FILTERS = {
  normal:   { }, // apa adanya, tanpa olahan
  beauty:   { skinSmooth: 0.45, tint: { color:'rgba(255,214,190,0.08)', mode:'soft-light' } },
  cleanskin:{ skinSmooth: 0.55, tint: { color:'rgba(232,240,247,0.10)', mode:'soft-light' }, brighten: 0.05 },
  softglow: { skinSmooth: 0.30, glow: 0.30, tint: { color:'rgba(255,235,225,0.06)', mode:'soft-light' } },
  ivory:    { tint: { color:'rgba(250,240,222,0.16)', mode:'soft-light' }, brighten: 0.06 },
  wedding:  { skinSmooth: 0.25, tint: { color:'rgba(230,180,140,0.16)', mode:'overlay' }, brighten: 0.03 },
};
...
function renderVideoFrame(){
  ...
  const recipe = VIDEO_FILTERS[state.videoFilter] || VIDEO_FILTERS.normal;

  // (1) Dasar: video asli penuh, tetap tajam (mata/alis/rambut/bibir jelas).
  drawVideoCover(videoRecCtx, videoRecPreview, REC_W, REC_H, mirror);

  // (2) Skin smooth / glow: butuh versi super-kecil->diperbesar sekali saja.
  if(recipe.skinSmooth || recipe.glow){
    drawVideoCover(_beautySmallCtx, videoRecPreview, BEAUTY_SMALL_W, BEAUTY_SMALL_H, mirror);
  }
  if(recipe.skinSmooth){
    videoRecCtx.save();
    videoRecCtx.globalAlpha = recipe.skinSmooth; // di bawah 0.6 supaya nggak flat/plastik
    videoRecCtx.imageSmoothingEnabled = true;
    videoRecCtx.imageSmoothingQuality = 'high';
    videoRecCtx.drawImage(_beautySmall, 0, 0, BEAUTY_SMALL_W, BEAUTY_SMALL_H, 0, 0, REC_W, REC_H);
    videoRecCtx.restore();
  }
  if(recipe.glow){
    videoRecCtx.save();
    videoRecCtx.globalAlpha = recipe.glow;
    videoRecCtx.globalCompositeOperation = 'screen'; // bikin highlight "mekar"/bloom
    videoRecCtx.imageSmoothingEnabled = true;
    videoRecCtx.imageSmoothingQuality = 'high';
    videoRecCtx.drawImage(_beautySmall, 0, 0, BEAUTY_SMALL_W, BEAUTY_SMALL_H, 0, 0, REC_W, REC_H);
    videoRecCtx.restore();
  ...
  // (3) Tint: sapuan warna tipis buat ubah mood keseluruhan.
  if(recipe.tint){
    videoRecCtx.save();
    videoRecCtx.globalCompositeOperation = recipe.tint.mode;
    videoRecCtx.fillStyle = recipe.tint.color;
    videoRecCtx.fillRect(0, 0, REC_W, REC_H);
    videoRecCtx.restore();
  }

  // (4) Brighten: sapuan putih tipis buat naikin kecerahan.
  if(recipe.brighten){
    videoRecCtx.save();
    videoRecCtx.globalAlpha = recipe.brighten;
    videoRecCtx.globalCompositeOperation = 'screen';
    videoRecCtx.fillStyle = '#ffffff';
    videoRecCtx.fillRect(0, 0, REC_W, REC_H);
    videoRecCtx.restore();
  }
}
