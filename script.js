...
function renderGalleryGrid(items){
  ...
  filtered.forEach(item=>{
    ...
    if(item.type === 'video'){
      // autoplay+muted+playsinline = wajib biar browser (termasuk iOS
      // Safari) mengizinkan video jalan sendiri sebagai preview di
      // grid tanpa perlu di-tap dulu, mirip preview Reels di profil IG.
      card.innerHTML = `<video src="${item.videoUrl}" muted playsinline autoplay loop preload="metadata"></video>
...
function openPhotoModal(item){
  ...
  if(item.type === 'video'){
    ...
    setVideoMuteIcon(false);
    dlBtn.href = item.videoUrl; dlBtn.setAttribute('download', 'wedding-memories-video.mp4');
  } else {
    modalVideo.pause();
...
/* ==================== VIDEO: kontrol ala Reels (tap buat play/pause, mute, progress tipis) ==================== */
function setVideoMuteIcon(muted){
  document.getElementById('videoMuteIcon').innerHTML = muted
...
(function setupReelsVideo(){
  const video = document.getElementById('modalVideo');
  const frame = document.getElementById('videoFrame');
  const tapLayer = document.getElementById('videoTapLayer');
...
function drawImageCover(ctx, imgOrDataUrl, dx, dy, dw, dh, filter){
  ...
  if(filter && filter !== 'natural' && filter !== 'original'){
    applyPhotoFilter(ctx, dx, dy, dw, dh, filter);
  }
}
/* ==================== 23 FILTER FOTO ====================
   Tiap filter didefinisikan lewat parameter numerik yang sama persis
   konsepnya dengan CSS filter (brightness/contrast/saturate/sepia/
   grayscale/hueRotate), lalu dihitung manual per-piksel (BUKAN pakai
   ctx.filter bawaan) — supaya hasilnya identik di semua browser
   termasuk Safari/iOS yang riwayatnya kurang konsisten soal ctx.filter.
   "fx" adalah efek tambahan (grain/vignette/glow/dst) yang digambar
   sebagai layer di atas, dieksekusi oleh applyFxOverlay(). */
const PHOTO_FILTERS = [
  { key:'natural',    name:'Natural',       brightness:1,     contrast:1,     saturate:1,     sepia:0,   grayscale:0, hueRotate:0,   fx:'' },
  { key:'beauty',     name:'Beauty',        brightness:1.08,  contrast:0.96,  saturate:1.08,  sepia:0,   grayscale:0, hueRotate:0,   fx:'glow' },
  { key:'clean',      name:'Clean Skin',    brightness:1.1,   contrast:0.95,  saturate:1.03,  sepia:0,   grayscale:0, hueRotate:0,   fx:'glow matte' },
  { key:'soft',       name:'Soft Glow',     brightness:1.12,  contrast:0.91,  saturate:1.05,  sepia:0,   grayscale:0, hueRotate:0,   fx:'glow' },
  { key:'ivory',      name:'Ivory',         brightness:1.11,  contrast:0.93,  saturate:0.96,  sepia:0.04,grayscale:0, hueRotate:0,   fx:'matte pearl' },
  { key:'wedding',    name:'Warm Wedding',  brightness:1.06,  contrast:1.02,  saturate:1.18,  sepia:0.14,grayscale:0, hueRotate:0,   fx:'warm' },
  { key:'champagne',  name:'Champagne',     brightness:1.08,  contrast:1.01,  saturate:1.12,  sepia:0.1, grayscale:0, hueRotate:0,   fx:'warm glow pearl' },
  { key:'peach',      name:'Peach Blush',   brightness:1.07,  contrast:1,     saturate:1.08,  sepia:0.09,grayscale:0, hueRotate:-6,  fx:'blush glow' },
  { key:'rose',       name:'Rose Garden',   brightness:1.06,  contrast:0.97,  saturate:1.08,  sepia:0,   grayscale:0, hueRotate:-10, fx:'blush fade' },
  { key:'pastel',     name:'Pastel Bloom',  brightness:1.12,  contrast:0.88,  saturate:0.92,  sepia:0,   grayscale:0, hueRotate:0,   fx:'fade glow pearl' },
  { key:'mocha',      name:'Mocha Matte',   brightness:1.02,  contrast:0.95,  saturate:0.92,  sepia:0.25,grayscale:0, hueRotate:0,   fx:'matte vignette' },
  { key:'filmgold',   name:'Film Gold',     brightness:1.04,  contrast:1.04,  saturate:1.22,  sepia:0.24,grayscale:0, hueRotate:0,   fx:'warm grain vignette' },
  { key:'sunnyfilm',  name:'Sunny Film',    brightness:1.11,  contrast:1.01,  saturate:1.18,  sepia:0.16,grayscale:0, hueRotate:0,   fx:'warm grain' },
  { key:'disposable', name:'Disposable',    brightness:1.03,  contrast:1.12,  saturate:1.3,   sepia:0.18,grayscale:0, hueRotate:0,   fx:'grain warm vignette' },
  { key:'vintage90',  name:'Vintage 90s',   brightness:1.08,  contrast:0.92,  saturate:0.88,  sepia:0.28,grayscale:0, hueRotate:0,   fx:'grain fade' },
  { key:'retrof',     name:'Retro Fade',    brightness:1.1,   contrast:0.88,  saturate:0.82,  sepia:0.12,grayscale:0, hueRotate:0,   fx:'fade matte' },
  { key:'digicam',    name:'Digicam',       brightness:1.09,  contrast:1.17,  saturate:1.24,  sepia:0,   grayscale:0, hueRotate:0,   fx:'grain cool vignette' },
  { key:'ccd',        name:'CCD Blue',      brightness:1.05,  contrast:1.13,  saturate:1.13,  sepia:0,   grayscale:0, hueRotate:-9,  fx:'cool grain' },
  { key:'silver',     name:'Silver Pearl',  brightness:1.08,  contrast:0.98,  saturate:0.95,  sepia:0,   grayscale:0, hueRotate:0,   fx:'cool pearl glow' },
  { key:'afterrain',  name:'After Rain',    brightness:1.04,  contrast:0.94,  saturate:0.95,  sepia:0,   grayscale:0, hueRotate:8,   fx:'cool fade pearl' },
  { key:'bw',         name:'B&W Classic',   brightness:1.04,  contrast:1.12,  saturate:1,     sepia:0,   grayscale:1, hueRotate:0,   fx:'grain vignette' },
  { key:'sepia',      name:'Sepia',         brightness:1.03,  contrast:1.02,  saturate:1,     sepia:0.82,grayscale:0, hueRotate:0,   fx:'grain' },
  { key:'midnight',   name:'Midnight',      brightness:0.92,  contrast:1.15,  saturate:0.92,  sepia:0,   grayscale:0, hueRotate:0,   fx:'cool vignette' },
];
function getPhotoFilter(key){ return PHOTO_FILTERS.find(f=>f.key===key) || PHOTO_FILTERS[0]; }

/* Matriks warna standar (formula resmi CSS Filter Effects spec) supaya
   hasilnya identik dengan filter CSS asli, walau dihitung manual. */
function applyBrightness(r,g,b,a){ return [r*a, g*a, b*a]; }
function applyContrast(r,g,b,a){ return [(r-128)*a+128, (g-128)*a+128, (b-128)*a+128]; }
function applySaturate(r,g,b,a){
  const lr=0.213, lg=0.715, lb=0.072;
  return [
    (lr+(1-lr)*a)*r + (lg-lg*a)*g + (lb-lb*a)*b,
    (lr-lr*a)*r + (lg+(1-lg)*a)*g + (lb-lb*a)*b,
    (lr-lr*a)*r + (lg-lg*a)*g + (lb+(1-lb)*a)*b,
  ];
}
...
function applyHueRotate(r,g,b,deg){
  ...
  const a1=0.213+c*0.787-s*0.213, a2=0.715-c*0.715-s*0.715, a3=0.072-c*0.072+s*0.928;
  const b1=0.213-c*0.213+s*0.143, b2=0.715+c*0.285+s*0.140, b3=0.072-c*0.072-s*0.283;
  const c1=0.213-c*0.213-s*0.787, c2=0.715-c*0.715+s*0.715, c3=0.072+c*0.928+s*0.072;
  return [ a1*r+a2*g+a3*b, b1*r+b2*g+b3*b, c1*r+c2*g+c3*b ];
}
/* Terapkan filter foto (per-piksel) ke area (x,y,w,h) di canvas. Urutan
   proses (brightness->contrast->saturate->sepia->grayscale->hue-rotate)
   sengaja dipakai konsisten untuk semua filter, supaya hasilnya stabil
   dan gampang diprediksi. */
function applyPhotoFilter(ctx, x, y, w, h, filterKey){
  if(!filterKey || filterKey === 'natural' || filterKey === 'original') return;
  const f = getPhotoFilter(filterKey);
  let imgData;
  ...
  for(let i=0; i<d.length; i+=4){
    ...
    if(f.brightness !== 1){ [r,g,b] = applyBrightness(r,g,b,f.brightness); }
    if(f.contrast !== 1){ [r,g,b] = applyContrast(r,g,b,f.contrast); }
    if(f.saturate !== 1){ [r,g,b] = applySaturate(r,g,b,f.saturate); }
    if(f.sepia){ [r,g,b] = applySepia(r,g,b,f.sepia); }
    if(f.grayscale){ [r,g,b] = applyGrayscale(r,g,b,f.grayscale); }
    if(f.hueRotate){ [r,g,b] = applyHueRotate(r,g,b,f.hueRotate); }
    d[i]=Math.max(0,Math.min(255,r)); d[i+1]=Math.max(0,Math.min(255,g)); d[i+2]=Math.max(0,Math.min(255,b));
  }
  ctx.putImageData(imgData, x, y);
  applyFxOverlay(ctx, x, y, w, h, f.fx);
}
/* Efek tambahan (fx) digambar sebagai layer tipis di atas foto yang
   sudah difilter — warna hangat/dingin, grain (bintik film), vignette
   (gelap di pinggir), glow (cahaya lembut), dst. Nama-nama efek & nilai
   opacity-nya mengikuti kombinasi yang sama seperti daftar 23 filter. */
function applyFxOverlay(ctx, x, y, w, h, fx){
  if(!fx) return;
  ctx.save();
  ctx.beginPath(); ctx.rect(x,y,w,h); ctx.clip();
  if(fx.includes('fade')){
    const grad = ctx.createLinearGradient(x,y,x+w,y+h);
    grad.addColorStop(0,'rgba(255,220,185,.08)'); grad.addColorStop(1,'rgba(110,70,100,.08)');
    ctx.fillStyle = grad; ctx.fillRect(x,y,w,h);
  }
  if(fx.includes('warm')){ ctx.fillStyle='rgba(255,181,90,.045)'; ctx.fillRect(x,y,w,h); }
  if(fx.includes('cool')){ ctx.fillStyle='rgba(80,155,255,.035)'; ctx.fillRect(x,y,w,h); }
  if(fx.includes('grain')){
    const count = Math.min(6000, Math.round(w*h/180));
  ...
  if(fx.includes('blush')){ ctx.fillStyle='rgba(255,170,190,.055)'; ctx.fillRect(x,y,w,h); }
  if(fx.includes('pearl')){ ctx.fillStyle='rgba(255,255,255,.04)'; ctx.fillRect(x,y,w,h); }
  if(fx.includes('matte')){ ctx.fillStyle='rgba(245,232,222,.06)'; ctx.fillRect(x,y,w,h); }
  if(fx.includes('vignette')){
    const grad = ctx.createRadialGradient(x+w/2,y+h/2,Math.min(w,h)*0.22,x+w/2,y+h/2,Math.max(w,h)*0.78);
    grad.addColorStop(0,'rgba(0,0,0,0)'); grad.addColorStop(1,'rgba(40,14,18,.22)');
    ctx.fillStyle = grad; ctx.fillRect(x,y,w,h);
  }
  if(fx.includes('glow')){
    const grad = ctx.createRadialGradient(x+w/2,y+h*0.34,Math.min(w,h)*0.05,x+w/2,y+h*0.34,Math.max(w,h)*0.58);
    grad.addColorStop(0,'rgba(255,245,232,.13)'); grad.addColorStop(0.45,'rgba(255,232,215,.08)'); grad.addColorStop(1,'rgba(255,255,255,0)');
    ctx.fillStyle = grad; ctx.fillRect(x,y,w,h);
  }
  ctx.restore();
}
...
reviewNextBtn.addEventListener('click', ()=>{ state.filter = 'natural'; renderFilterScreen(); showScreen('filterscreen'); });

/* ==================== SCREEN: FILTER / COMPOSE ==================== */
const filterPills = document.getElementById('filterPills');
const composeCanvas = document.getElementById('composeCanvas');
const FILTERS = PHOTO_FILTERS.map(f => [f.key, f.name]);
...
/* ==================== 23 FILTER VIDEO (versi real-time) ====================
   PENTING: ini BUKAN hasil sama persis kayak filter foto. Video harus
   diproses tiap frame (30-60x per detik), jadi perhitungan per-piksel
   penuh (seperti di foto) akan terlalu berat & bikin video patah-patah
   di HP yang kurang kencang. Solusinya: dekati karakter warnanya lewat
   "tint" (warna transparan) yang ditumpuk pakai mode blend canvas
   (overlay/screen/multiply/dst) — jauh lebih ringan, hanya 'bw' yang
   tetap perlu proses per-piksel (khusus itu saja) karena butuh
   desaturasi presisi yang tidak bisa didekati pakai tint biasa. */
const VIDEO_FILTERS = {
  natural:    { layers: [] },
  beauty:     { layers: [ { mode:'screen',     color:'rgba(255,250,240,0.08)' }, { mode:'overlay', color:'rgba(255,245,235,0.05)' } ] },
  clean:      { layers: [ { mode:'screen',     color:'rgba(255,255,255,0.08)' }, { mode:'soft-light', color:'rgba(245,240,235,0.06)' } ] },
  soft:       { layers: [ { mode:'screen',     color:'rgba(255,245,235,0.10)' }, { mode:'overlay', color:'rgba(255,240,230,0.05)' } ] },
  ivory:      { layers: [ { mode:'overlay',    color:'rgba(250,240,222,0.18)' }, { mode:'screen',  color:'rgba(255,255,255,0.05)' } ] },
  wedding:    { layers: [ { mode:'overlay',    color:'rgba(220,160,90,0.14)'  }, { mode:'screen',  color:'rgba(255,225,180,0.06)' } ] },
  champagne:  { layers: [ { mode:'overlay',    color:'rgba(230,190,120,0.12)' }, { mode:'screen',  color:'rgba(255,240,205,0.07)' } ] },
  peach:      { layers: [ { mode:'overlay',    color:'rgba(255,190,170,0.12)' }, { mode:'screen',  color:'rgba(255,220,205,0.05)' } ] },
  rose:       { layers: [ { mode:'overlay',    color:'rgba(255,175,195,0.12)' } ] },
  pastel:     { layers: [ { mode:'screen',     color:'rgba(255,255,255,0.10)' }, { mode:'soft-light', color:'rgba(230,225,245,0.08)' } ] },
  mocha:      { layers: [ { mode:'multiply',   color:'rgba(170,140,110,0.14)' }, { mode:'soft-light', color:'rgba(120,95,75,0.06)' } ] },
  filmgold:   { layers: [ { mode:'overlay',    color:'rgba(200,150,80,0.16)' }, { mode:'multiply', color:'rgba(255,220,160,0.08)' } ] },
  sunnyfilm:  { layers: [ { mode:'overlay',    color:'rgba(255,200,110,0.14)' }, { mode:'screen', color:'rgba(255,235,180,0.07)' } ] },
  disposable: { layers: [ { mode:'overlay',    color:'rgba(255,190,120,0.16)' }, { mode:'multiply', color:'rgba(70,50,40,0.08)' } ] },
  vintage90:  { layers: [ { mode:'multiply',   color:'rgba(150,120,90,0.14)' }, { mode:'soft-light', color:'rgba(210,190,150,0.08)' } ] },
  retrof:     { layers: [ { mode:'soft-light', color:'rgba(200,180,160,0.14)' }, { mode:'screen', color:'rgba(255,245,230,0.05)' } ] },
  digicam:    { layers: [ { mode:'hard-light', color:'rgba(90,150,255,0.05)' }, { mode:'overlay', color:'rgba(255,255,255,0.04)' } ] },
  ccd:        { layers: [ { mode:'overlay',    color:'rgba(90,150,230,0.12)' }, { mode:'screen', color:'rgba(160,200,255,0.05)' } ] },
  silver:     { layers: [ { mode:'screen',     color:'rgba(220,230,240,0.08)' }, { mode:'soft-light', color:'rgba(200,210,220,0.05)' } ] },
  afterrain:  { layers: [ { mode:'overlay',    color:'rgba(120,160,200,0.10)' }, { mode:'screen', color:'rgba(200,220,235,0.05)' } ] },
  bw:         { layers: [], pixelGrayscale: true }, // ditangani khusus, lihat renderVideoFrame
  sepia:      { layers: [ { mode:'multiply',   color:'rgba(150,110,70,0.30)' }, { mode:'soft-light', color:'rgba(210,175,120,0.12)' } ] },
  midnight:   { layers: [ { mode:'multiply',   color:'rgba(60,70,100,0.20)' }, { mode:'hard-light', color:'rgba(40,45,70,0.08)' } ] },
};
...
function renderVideoFrame(){
  ...
  if(recipe.pixelGrayscale){
    // Khusus B&W: satu-satunya filter video yang butuh proses per-piksel
    // (desaturasi presisi tidak bisa didekati pakai tint/blend biasa).
    // Diproses cuma saat filter ini aktif, jadi tidak membebani filter lain.
    try{
      const imgData = videoRecCtx.getImageData(0, 0, REC_W, REC_H);
      const d = imgData.data;
      for(let i=0; i<d.length; i+=4){
        const gray = d[i]*0.213 + d[i+1]*0.715 + d[i+2]*0.072;
        const c = (gray-128)*1.12 + 128; // sedikit tambah kontras, senada resep foto
        d[i] = d[i+1] = d[i+2] = Math.max(0, Math.min(255, c));
      }
      videoRecCtx.putImageData(imgData, 0, 0);
    }catch(e){ /* kalau gagal (mis. canvas ke-taint), biarkan lewat tanpa efek */ }
  }
...
/* Generate tombol filter video secara OTOMATIS dari daftar 23 filter
   yang sama seperti mode foto (PHOTO_FILTERS), supaya nama & urutannya
   selalu konsisten di kedua mode. Tombol pertama (natural) otomatis
   aktif duluan. Titik warna kecil (dot) tiap tombol dibikin otomatis
   dari warna tint filter terkait, biar tidak perlu bikin CSS class
   baru satu-satu untuk 23 filter. */
reelsFilterRowEl.innerHTML = PHOTO_FILTERS.map((f, i) => {
  const vf = VIDEO_FILTERS[f.key];
  const dotColor = (vf && vf.layers && vf.layers[0]) ? vf.layers[0].color : (f.key==='bw' ? 'rgb(160,160,160)' : '#fff');
  return `<button type="button" class="reels-filter-btn reels-filter-main${i===0?' selected':''}" data-filter="${f.key}">`
       + `<span class="fdot" style="background:${dotColor}"></span>`
       + `<span class="flabel">${f.name}</span></button>`;
}).join('');
