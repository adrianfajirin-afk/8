...
async function fetchGitHubGallery(){
  ...
  // manifest.json dibaca langsung dari raw.githubusercontent.com (repo public, tanpa token,
  // dan tidak kena limit rate lebih ketat milik api.github.com)
  ...
  try{
    ...
    if(!res.ok){
      if(res.status !== 404) console.error('[GitHub] Gagal ambil manifest:', res.status);
