...
const SCREENS_WITH_BOTTOM_NAV = ['welcome', 'gallery'];
  ...
  feedObserver = new IntersectionObserver((entries)=>{
    entries.forEach(entry=>{
      ...
      if(entry.isIntersecting && entry.intersectionRatio >= 0.6){
        ...
        if(post._audioEl){ post._audioEl.currentTime = 0; post._audioEl.play().catch(()=>{}); }
      } else {
        if(post._videoEl){ post._videoEl.pause(); }
        if(post._audioEl){ post._audioEl.pause(); }
      }
    });
...
function renderStoriesRow(items){
  ...
  items.forEach((item, idx)=>{
    ...
    const name = item.name || 'Anonim';
    const thumbTag = item.type === 'video'
      ? `<video src="${item.videoUrl}" muted playsinline autoplay loop preload="metadata"></video>`
      : `<img src="${item.img}" loading="lazy">`;
    bubble.innerHTML = `
      <div class="story-ring"><div class="story-ring-inner">${thumbTag}</div></div>
...
function stopStoryMedia(){
  ...
  if(storyAudioEl){ storyAudioEl.pause(); storyAudioEl = null; }
...
function showStoryAt(idx){
  ...
  storyAvatarEl.innerHTML = item.type === 'video'
    ? `<video src="${item.videoUrl}" muted playsinline></video>`
    : `<img src="${item.img}">`;
  ...
  if(item.type === 'video'){
    ...
    if(item.audioUrl){
      storyAudioEl = new Audio(item.audioUrl);
      storyAudioEl.play().catch(()=>{});
    }
