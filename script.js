...
// Resolusi rekaman ditetapkan (bukan resolusi kamera asli) — dijaga sedang
// (bukan full-HD) supaya proses "Beauty" (butuh gambar diolah tiap frame)
// tetap ringan/lancar di HP menengah, bukan cuma di HP kencang.
const REC_W = 480, REC_H = 854; // rasio 9:16
...
const BEAUTY_SMALL_W = Math.round(REC_W/7), BEAUTY_SMALL_H = Math.round(REC_H/7);
...
function renderVideoFrame(){
  ...
  if(state.videoFilter === 'beauty'){
    // Downscale jauh lalu digambar besar lagi -> hasil alami lebih lembut
    // (mirip efek "beauty" murah di banyak app), tanpa loop per-piksel.
    ...
    // Lapisan cahaya + hangat tipis (murah — cuma 2x fillRect, bukan olah piksel)
    videoRecCtx.save();
    videoRecCtx.globalCompositeOperation = 'overlay';
    videoRecCtx.fillStyle = 'rgba(255,255,255,0.10)';
    videoRecCtx.fillRect(0, 0, REC_W, REC_H);
    videoRecCtx.globalCompositeOperation = 'soft-light';
    videoRecCtx.fillStyle = 'rgba(255,205,175,0.14)';
    videoRecCtx.fillRect(0, 0, REC_W, REC_H);
    videoRecCtx.restore();
  } else {
