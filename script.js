...
document.getElementById('shareOnlyBtn').addEventListener('click', async ()=>{
  ...
  downloadCanvas(canvas, filename); // pakai blob: URL, aman dipanggil langsung tanpa menunggu upload
  renderGallery();
  showScreen('gallery');
...
document.getElementById('finalShareBtn').addEventListener('click', async ()=>{
  ...
  // Download foto & audio pakai blob: URL (bukan data: URI), jadi aman
  // dipanggil langsung tanpa perlu menunggu upload selesai — blob: URL
  // tidak membuat Safari "berpindah halaman" seperti data: URI dulu.
