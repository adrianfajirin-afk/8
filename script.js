/* =====================================================================
   ...
   5) MUSIK LATAR OTOMATIS (music/bgm-1.mp3 ... music/bgm-15.mp3)
      ...
      - Otomatis muter pas fotonya kelihatan di layar (grid galeri, feed,
        modal, atau story viewer), dan otomatis berhenti begitu foto lain
        yang mulai keputer duluan (nggak bakal numpuk beberapa lagu
        sekaligus).
   ===================================================================== */
...
function renderGalleryGrid(items){
  ...
  galleryGridObserver = new IntersectionObserver((entries)=>{
    entries.forEach(entry=>{
      const audioEl = entry.target._audioEl;
      if(!audioEl) return;
      if(entry.isIntersecting && entry.intersectionRatio >= 0.6){
        audioEl.currentTime = 0; playAmbientAudio(audioEl);
      } else {
        pauseAmbientAudio(audioEl);
      }
    });
  }, { threshold:[0, 0.6, 1] });
  ...
  filtered.forEach(item=>{
    ...
    if(item.type === 'video'){
      ...
      if(audioUrl){
        card._audioEl = new Audio(audioUrl);
        card._audioEl.loop = true;
        galleryGridObserver.observe(card);
      }
    }
