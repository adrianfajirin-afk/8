...
function showScreen(name){
  // PENTING: kalau lagi ada musik/VN yang diputer (baik dari preview VN,
  // feed foto, dll) terus user pindah ke screen lain, itu HARUS di-stop
  // di sini. Sebelumnya cuma di-handle pas modal/story ditutup atau pas
  // keluar app doang — jadi kalau pindah antar-screen dalam app pake
  // tombol balik/menu, suaranya kebawa terus ke mana-mana. pauseAllMedia()
  // udah nyimpen semua elemen audio/video yang pernah dibikin, jadi aman
  // dipanggil di sini walau lagi gak ada yang muter apa-apa.
  pauseAllMedia();
...
/* Penjaga "1 suara aktif dalam satu waktu" LINTAS TEMPAT (grid galeri,
   ...
   Sekarang transisinya pake FADE (naik/turun volume pelan-pelan, bukan
   langsung putus/nyala) biar kerasa mulus kayak feed Reels/IG, bukan
   "klik" mendadak tiap digeser dikit. */
let _activeAmbientAudio = null;
const _fadeTimers = new WeakMap(); // audioEl -> id requestAnimationFrame yang lagi jalan
const FADE_MS = 260;

function _stopFade(audioEl){
  const id = _fadeTimers.get(audioEl);
  if(id) cancelAnimationFrame(id);
  _fadeTimers.delete(audioEl);
}
function _fadeVolume(audioEl, target, onDone){
  _stopFade(audioEl);
  ...
  function step(now){
    ...
    if(p < 1){
      _fadeTimers.set(audioEl, requestAnimationFrame(step));
    } else {
      _fadeTimers.delete(audioEl);
      if(onDone) onDone();
    }
...
function playAmbientAudio(audioEl){
  if(!audioEl) return;
  if(_activeAmbientAudio && _activeAmbientAudio !== audioEl) pauseAmbientAudio(_activeAmbientAudio);
  _activeAmbientAudio = audioEl;
  _stopFade(audioEl);
  audioEl.volume = 0;
  audioEl.play().catch(()=>{});
  _fadeVolume(audioEl, 1);
}
function pauseAmbientAudio(audioEl){
  if(!audioEl) return;
  if(_activeAmbientAudio === audioEl) _activeAmbientAudio = null;
  _fadeVolume(audioEl, 0, ()=>{ audioEl.pause(); });
}

/* ==================== MUTE SEMUA PAS USER KELUAR/PINDAH APP ====================
   Musik BGM, VN, dan video dibikin lewat `new Audio()`/<video> yang diputer
   manual pake JS — browser TIDAK otomatis diemin ini pas user kunci layar,
   pindah ke app lain, atau pindah tab. Makanya perlu di-pause manual pas
   halaman ini "disembunyikan" (visibilitychange) ATAU pas beneran ditinggal
   (pagehide, buat jaga-jaga di Safari/iOS yang kadang gak selalu ngasih
   tau lewat visibilitychange pas app di-swipe-close).
   _allAudioEls nyimpen SEMUA elemen <audio> yang pernah dibikin di app ini
   (musik BGM per foto di feed, musik di story viewer, preview VN pas bikin
   harapan) — soalnya elemen-elemen ini gak ditaro di DOM, jadi gak kejaring
   kalau cuma querySelectorAll('audio'). Video (feed & story) ada di DOM,
   jadi itu langsung di-query aja. */
const _allAudioEls = new Set();
function _registerAudioEl(audioEl){
  if(audioEl) _allAudioEls.add(audioEl);
}
function pauseAllMedia(){
  _activeAmbientAudio = null;
  // Biar pas balik lagi ke tab/app ini, foto yang lagi kebuka di feed mau
  // gak mau di-anggap "belum aktif" lagi, jadi musiknya beneran dicek &
  // dinyalain ulang (bukan dianggep "masih aktif" padahal tadi udah
  // dipause paksa di sini).
  _feedActivePost = null;
  _allAudioEls.forEach(el=>{
    _stopFade(el);
  ...
  document.querySelectorAll('.ig-vn-play-btn.playing').forEach(b=> b.classList.remove('playing'));
  if(typeof finalPlaying !== 'undefined' && finalPlaying){
    finalPlaying = false;
    if(typeof finalPlayIcon !== 'undefined' && finalPlayIcon){
      finalPlayIcon.innerHTML = '<path d="M8 5v14l11-7z"/>';
    }
  }
}
...
let _feedActivePost = null;

/* Cache rasio asli tiap foto (URL -> "w / h"), biar begitu feed di-render
   ULANG (misal buka-tutup modal berkali-kali), kotak foto langsung tau
   tinggi yang bener dari awal tanpa nunggu foto ke-load lagi -> gak ada
   "nyentak" pas scroll. Foto yang BENER-BENER baru pertama kali ke-load
   (belum pernah masuk cache ini) tetap dapet tebakan aspect-ratio default
   dari CSS dulu, baru di-update pas event 'load' beres. */
const _photoRatioCache = new Map();
/* Sama kayak _photoRatioCache di atas, tapi buat VIDEO: nyimpen rasio asli
   tiap video (URL -> "w / h") biar kotak videonya BENERAN ngikutin rasio
   ASLI hasil rekaman (16:9, 9:16, atau apapun apa adanya), bukan dipatok
   9:16 kayak sebelumnya. Video yang rasio aslinya beda dari 9:16 (misal
   16:9) sebelumnya kepotong parah karena dipaksa masuk kotak 9:16 -- sekarang
   kotaknya yang nyesuain ke rasio videonya, bukan sebaliknya. */
const _videoRatioCache = new Map();

/* Hentikan VN yang lagi diputer di 1 post (kalau ada) & balikin tombolnya
   ke tampilan diam. Dipanggil pas post-nya discroll-lewatin, feed
   di-render ulang, atau modal ditutup -- supaya VN gak "nyangkut" muter
   terus padahal tamu udah pindah/nutup, walau VN emang sengaja TIDAK
   autoplay pas post baru muncul. */
function stopPostVn(post){
  if(post._vnBtn && post._vnBtn._stopVn) post._vnBtn._stopVn();
}
...
function buildFeedPost(item){
  ...
  let videoEl = null, audioEl = null, vnAudioEl = null;
  // VN ASLI dari tamu (item.audioUrl) BEDA perlakuan sama musik latar (BGM)
  // otomatis: VN TIDAK autoplay, cuma diputer kalau tombol VN di baris
  // aksi (sebelah ikon komentar) di-tap manual. BGM otomatis tetap
  // autoplay ngikut scroll kayak sebelumnya (lihat activatePost).
  const hasRealVn = item.type !== 'video' && !!item.audioUrl;

  if(item.type === 'video'){
    // Sama pola kayak foto: kalau video ini udah pernah ke-load sebelumnya
    // (rasio aslinya udah ke-cache), langsung pasang aspect-ratio yang
    // BENER dari awal, SEBELUM videonya mulai dirender -- supaya kotaknya
    // dari awal udah pas sama rasio ASLI video (16:9, 9:16, dll, apa
    // adanya), bukan dipaksa 9:16 terus dikepotong.
    const cachedVRatio = _videoRatioCache.get(item.videoUrl);
    media.innerHTML = `
      <div class="ig-video-frame"${cachedVRatio ? ` style="aspect-ratio:${cachedVRatio};"` : ''}>
    ...
    videoEl.muted = false;
    // Rasio aslinya baru pasti kebaca begitu metadata videonya kelar
    // dimuat -- disimpen ke cache buat dipakai LANGSUNG pas render
    // berikutnya (buka-tutup modal lagi dll), bukan diubah on-the-fly
    // sekarang juga (biar gak "nyentak" pas lagi di area scroll-snap,
    // sama alasannya kayak foto di atas).
    videoEl.addEventListener('loadedmetadata', function(){
      if(videoEl.videoWidth && videoEl.videoHeight){
        _videoRatioCache.set(item.videoUrl, `${videoEl.videoWidth} / ${videoEl.videoHeight}`);
      }
    }, { once:true });
    ...
    img.loading = 'lazy';
    img.decoding = 'async';
    // Kalau foto ini udah pernah ke-load sebelumnya (cache ada), langsung
    // pasang rasio aslinya SEKARANG JUGA, SEBELUM foto mulai di-render —
    // biar ukuran kotaknya udah pasti dari awal.
    // PENTING: kalau belum pernah ke-load (belum ada di cache), kita
    // BIARIN aja pake tebakan default dari CSS dan TIDAK diubah lagi
    // setelah itu (cuma tebakannya kepakai selamanya buat post ini,
    // foto ditampilin utuh tanpa kepotong berkat object-fit:contain).
    // Sengaja gak di-update on-the-fly pas 'load' kayak sebelumnya,
    // soalnya ganti aspect-ratio SAAT lagi di dalam area scroll-snap
    // bikin scroll-snap engine-nya "kaget"/nyentak lagi — di Chrome
    // Android efeknya kecil, tapi di Safari malah jadi lebih parah.
    // Rasio aslinya tetap kesimpen ke cache di bawah, jadi begitu foto
    // yang sama nongol lagi di render berikutnya (buka modal lagi dll),
    // kotaknya udah langsung pas dari awal, gak nunggu 'load' lagi.
    const cachedRatio = _photoRatioCache.get(item.img);
    if(cachedRatio) media.style.aspectRatio = cachedRatio;
    img.addEventListener('load', ()=>{
      if(img.naturalWidth && img.naturalHeight){
        _photoRatioCache.set(item.img, `${img.naturalWidth} / ${img.naturalHeight}`);
      }
    }, { once:true });
    img.src = item.img;
    media.appendChild(img);
    // Cuma bikin elemen audio ambient (autoplay ngikut scroll) buat foto
    // yang BUKAN VN asli (musik latar otomatis). VN asli TIDAK dibikin di
    // sini sama sekali -> gak ada jalan buat dia autoplay lewat sini.
    if(!hasRealVn && resolveAudioUrlFor(item)){
      audioEl = new Audio(resolveAudioUrlFor(item));
      audioEl.loop = true;
      _registerAudioEl(audioEl);
    }
  }
  ...
  // ==================== PEMUTAR VN (di samping ikon komentar) ====================
  // Cuma muncul kalau postingan ini emang punya VN ASLI dari tamu. TIDAK
  // pernah autoplay -- baru mulai muter pas tombol play di-tap manual.
  // Tampilannya pill lengkap ala WhatsApp: tombol play/pause bulat, angka
  // durasi, dan garis titik-titik "gelombang" yang keisi (nandain posisi
  // main) selagi VN diputer.
  //
  // FIX JEDA LAMA SEBELUM SUARA KELUAR: elemen audio-nya dibikin & mulai
  // di-preload (preload="auto") dari SAAT postingan ini dirender ke feed
  // (bukan baru dibikin pas tombol ditekan), jadi pas tombol beneran
  // di-tap datanya udah/lagi di-download di background dan biasanya
  // langsung siap muter. Selama masih beneran nunggu (jaringan lambat),
  // tombolnya nampilin animasi loading (cincin muter).
  if(hasRealVn){
    vnAudioEl = new Audio(item.audioUrl);
    vnAudioEl.preload = 'auto';
    _registerAudioEl(vnAudioEl);
    post._vnAudioEl = vnAudioEl;

    const DOT_COUNT = 26;
    const dotsHtml = Array.from({length: DOT_COUNT}, ()=> '<span></span>').join('');

    const vnPill = document.createElement('div');
    vnPill.className = 'ig-vn-pill';
    vnPill.innerHTML = `
      <button class="ig-vn-play-btn" aria-label="Putar suara harapan">
        <svg class="ig-vn-play-icon" viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg>
        <svg class="ig-vn-pause-icon" viewBox="0 0 24 24"><path d="M6 5h4v14H6zM14 5h4v14h-4z"/></svg>
        <span class="ig-vn-spinner"></span>
      </button>
      <div class="ig-vn-dur">${item.audioDurationLabel || '00:00'}</div>
      <div class="ig-vn-dots">${dotsHtml}</div>`;
    actions.appendChild(vnPill);

    const vnBtn = vnPill.querySelector('.ig-vn-play-btn');
    const vnDurEl = vnPill.querySelector('.ig-vn-dur');
    const vnDots = Array.from(vnPill.querySelectorAll('.ig-vn-dots span'));
    const vnBaseDurLabel = item.audioDurationLabel || '00:00';

    function setVnProgress(ratio){
      const filled = Math.round(Math.max(0, Math.min(1, ratio)) * vnDots.length);
      vnDots.forEach((dot, i)=> dot.classList.toggle('played', i < filled));
    }

    const stopThisVn = function(){
      vnAudioEl.pause();
      vnBtn.classList.remove('playing', 'loading');
    };
    vnPill._stopVn = stopThisVn;

    vnAudioEl.addEventListener('waiting', function(){ vnBtn.classList.add('loading'); });
    vnAudioEl.addEventListener('playing', function(){ vnBtn.classList.remove('loading'); });
    vnAudioEl.addEventListener('timeupdate', function(){
      if(vnAudioEl.duration){
        setVnProgress(vnAudioEl.currentTime / vnAudioEl.duration);
        vnDurEl.textContent = fmtTime(vnAudioEl.currentTime);
      }
    });
    vnAudioEl.addEventListener('ended', function(){
      vnBtn.classList.remove('playing', 'loading');
      setVnProgress(0);
      vnDurEl.textContent = vnBaseDurLabel;
    });

    vnBtn.addEventListener('click', function(ev){
      ev.stopPropagation();
      if(vnBtn.classList.contains('playing')){
        stopThisVn();
      } else {
        // 1 suara aktif dalam satu waktu: pause musik latar & VN lain yang
        // kebetulan lagi jalan (termasuk VN di post lain).
        if(_activeAmbientAudio) pauseAmbientAudio(_activeAmbientAudio);
        document.querySelectorAll('.ig-vn-pill').forEach(function(p){
          if(p !== vnPill && p._stopVn) p._stopVn();
        });
        vnBtn.classList.add('playing');
        // Kalau datanya belum cukup buat langsung jalan mulus, tunjukin
        // loading dulu sampai event 'playing' beneran nyala (lihat atas).
        if(vnAudioEl.readyState < 3) vnBtn.classList.add('loading');
        vnAudioEl.play().catch(function(){ vnBtn.classList.remove('playing', 'loading'); });
      }
    });
    post._vnBtn = { _stopVn: stopThisVn };
  }
...
function renderFeed(items){
  // Jaga-jaga: kalau ternyata renderFeed dipanggil lagi padahal feed
  // sebelumnya masih ada yang muter (post lama belum sempet di-pause,
  // misal lewat jalur yang belum ke-cover), pause dulu manual di sini
  // SEBELUM innerHTML-nya dibuang — soalnya kalau nggak, objek Audio-nya
  // tetep nyangkut di memori dan terus muter walau elemen postingannya
  // udah kehapus dari layar (makanya bisa kerasa "suara masih ikut").
  ...
  if(_feedScrollHandler){ feedContainer.removeEventListener('scroll', _feedScrollHandler); _feedScrollHandler = null; }
  _feedActivePost = null;

  const posts = items.map(item => buildFeedPost(item));
  posts.forEach(p => feedContainer.appendChild(p));

  /* ==================== KONTROL PLAY/PAUSE FEED (bukan IntersectionObserver) ====================
     Sebelumnya ini pakai IntersectionObserver, tapi di Safari observer-nya
     suka ngasih update yang gak halus/gak lengkap pas scroll pake root
     berupa div yang bisa di-scroll (bukan viewport) — makanya kejadiannya
     "scroll dikit langsung berhenti, scroll dikit lagi malah restart".
     Sekarang kita ukur sendiri manual pake getBoundingClientRect() tiap
     scroll (di-throttle lewat requestAnimationFrame biar ringan), jadi
     hasilnya konsisten di semua browser, gak gantung ke observer siapa
     pun. Ada 2 batas berbeda (hysteresis) biar gak "kedap-kedip":
       - Post BARU dianggap "aktif" (mulai diputer) kalau kelihatan >=60%.
       - Post yang LAGI aktif baru dianggap "ditinggal" (di-pause) kalau
         kelihatannya udah turun sampai di bawah 35% — jadi geser dikit
         doang gak bakal langsung matiin/restart musiknya. */
  function visibleRatio(el, containerRect){
  ...
  function activatePost(post){
    if(_feedActivePost === post) return;
    if(_feedActivePost){
      if(_feedActivePost._videoEl) _feedActivePost._videoEl.pause();
      if(_feedActivePost._audioEl) pauseAmbientAudio(_feedActivePost._audioEl);
      stopPostVn(_feedActivePost);
    }
    _feedActivePost = post;
    if(post){
      if(post._videoEl && post._videoEl.paused) post._videoEl.play().catch(()=>{});
      if(post._audioEl){
        // Reset ke awal cuma pas PERTAMA KALI lagu ini keputer di post
        // ini — bukan tiap kali post-nya jadi aktif lagi, biar gak
        // "restart mulu" pas sekadar scroll maju-mundur dikit.
        if(!post._audioStarted){ post._audioEl.currentTime = 0; post._audioStarted = true; }
        playAmbientAudio(post._audioEl);
      }
    }
  }

  let rafPending = false;
  function computeActivePost(){
    rafPending = false;
    if(!feedContainer.isConnected) return;
    const containerRect = feedContainer.getBoundingClientRect();

    let bestPost = null, bestRatio = 0;
    posts.forEach(post=>{
      const ratio = visibleRatio(post, containerRect);
      if(ratio > bestRatio){ bestRatio = ratio; bestPost = post; }
    });

    if(_feedActivePost){
      const activeRatio = visibleRatio(_feedActivePost, containerRect);
      if(activeRatio < 0.35){
        // Post yang lagi aktif udah beneran mau ilang -> pindah ke yang
        // paling kelihatan sekarang (kalau ada yang cukup dominan), atau
        // berhenti total kalau emang gak ada post yang kelihatan jelas.
        activatePost(bestRatio >= 0.5 ? bestPost : null);
      }
      // else: post yang aktif masih cukup kelihatan (>=35%) -> BIARIN
      // aja, gak usah gonta-ganti walau ada post lain yang sekarang
      // kebetulan lebih kelihatan dikit (menghindari kedap-kedip pas
      // pas-pasan di tengah antara 2 foto).
    } else if(bestRatio >= 0.5){
      activatePost(bestPost);
    }
  }

  function onScroll(){
    if(!rafPending){ rafPending = true; requestAnimationFrame(computeActivePost); }
  }
  feedContainer.addEventListener('scroll', onScroll, { passive:true });
  _feedScrollHandler = onScroll;

  // Hitung posisi awal (misal abis buka modal & lompat ke foto tertentu).
  requestAnimationFrame(computeActivePost);
...
function openPhotoModal(item){
  ...
  // Setelah lompat/reset posisi scroll di atas, ukur ulang post mana yang
  // aktif (scrollIntoView di sebagian browser gak langsung mentrigger
  // event 'scroll' dalam frame yang sama).
  requestAnimationFrame(()=>{
    if(_feedScrollHandler) _feedScrollHandler();
  });
}
function closeFeed(){
