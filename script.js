...
async function renderGallery(){
  ...
  items.forEach(item=>{
    ...
    if(item.type === 'video'){
      card.innerHTML = `<video src="${item.videoUrl}" muted preload="metadata" loading="lazy"></video>
        <div class="video-badge"><svg viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg></div>
        <div class="meta"><span>${item.name || 'Anonim'}</span></div>`;
    } else {
...
/* ==================== RESEP 23 FILTER VIDEO ====================
   PENTING: semua filter di sini SENGAJA tidak pakai teknik blur/downscale
   apa pun (tidak ada downsample->upsample, tidak ada ctx.filter blur).
   Efeknya murni "color grading" — sapuan warna & kontras lewat canvas
   composite blend mode di atas frame video ASLI apa adanya, jadi hasilnya
   tetap tajam/tegas, bukan lembek/soft-focus.
   Tiap resep = daftar "layer" yang ditumpuk berurutan, tiap layer:
     { mode: <globalCompositeOperation>, color: <rgba(...)>, alpha?: <0-1> }
   - mode 'saturation' + warna abu-abu netral = kontrol saturasi (0 = B&W).
   - mode 'color' + warna tertentu = ganti hue/saturasi tapi luminositas
     (gelap-terang) tetap ikut video asli -> pas buat tone sepia/duotone.
   - mode 'overlay' / 'hard-light' = tint + kontras yang kerasa "punchy".
   - mode 'multiply' = gelapin/kasih mood berat (shadow).
   - mode 'screen' = terangin highlight / kesan pudar-hangat ala film.
   Alpha di tiap layer ngatur seberapa kerasa efeknya (opsional, default 1). */
const VIDEO_FILTERS = {
  natural:   { layers: [] }, // apa adanya, tanpa olahan
  beauty:    { layers: [
    { mode:'screen',  color:'rgba(255,255,255,0.05)' },
    { mode:'overlay', color:'rgba(255,205,185,0.14)' },
  ]},
  clean:     { layers: [
    { mode:'saturation', color:'rgb(225,225,225)', alpha:0.35 },
    { mode:'screen',     color:'rgba(235,245,255,0.08)' },
  ]},
  soft:      { layers: [
    { mode:'screen',  color:'rgba(255,240,225,0.16)' },
    { mode:'overlay', color:'rgba(255,225,205,0.10)' },
  ]},
  ivory:     { layers: [
  ...
  wedding:   { layers: [
    { mode:'overlay', color:'rgba(230,180,140,0.22)' },
    { mode:'screen',  color:'rgba(255,235,210,0.06)' },
  ]},
  champagne: { layers: [
    { mode:'overlay', color:'rgba(230,205,160,0.20)' },
    { mode:'screen',  color:'rgba(255,248,225,0.08)' },
  ]},
  peach:     { layers: [
    { mode:'overlay',    color:'rgba(255,190,170,0.20)' },
    { mode:'saturation', color:'rgb(255,180,170)', alpha:0.18 },
  ]},
  rose:      { layers: [
    { mode:'overlay',  color:'rgba(220,140,150,0.22)' },
    { mode:'multiply', color:'rgba(255,225,230,0.12)' },
  ]},
  pastel:    { layers: [
    { mode:'saturation', color:'rgb(230,230,230)', alpha:0.30 },
    { mode:'screen',     color:'rgba(240,225,245,0.14)' },
  ]},
  mocha:     { layers: [
    { mode:'saturation', color:'rgb(200,200,200)', alpha:0.35 },
    { mode:'multiply',   color:'rgba(150,110,90,0.20)' },
    { mode:'screen',     color:'rgba(210,190,175,0.06)' },
  ]},
  filmgold:  { layers: [
    { mode:'overlay', color:'rgba(215,170,90,0.22)' },
    { mode:'screen',  color:'rgba(255,225,170,0.08)' },
  ]},
  sunnyfilm: { layers: [
    { mode:'screen',  color:'rgba(255,220,150,0.20)' },
    { mode:'overlay', color:'rgba(255,190,110,0.12)' },
  ]},
  disposable:{ layers: [
    { mode:'overlay',    color:'rgba(230,220,160,0.18)' },
    { mode:'saturation', color:'rgb(210,210,210)', alpha:0.20 },
    { mode:'screen',     color:'rgba(255,255,255,0.07)' },
  ]},
  vintage90: { layers: [
    { mode:'screen',     color:'rgba(255,235,200,0.18)' },
    { mode:'overlay',    color:'rgba(220,170,120,0.16)' },
    { mode:'saturation', color:'rgb(215,215,215)', alpha:0.20 },
  ]},
  retrof:    { layers: [
    { mode:'screen',     color:'rgba(230,225,215,0.22)' },
    { mode:'saturation', color:'rgb(210,210,210)', alpha:0.35 },
  ]},
  digicam:   { layers: [
    { mode:'saturation', color:'rgb(255,80,80)', alpha:0.15 },
    { mode:'overlay',    color:'rgba(200,220,255,0.12)' },
  ]},
  ccd:       { layers: [
    { mode:'overlay',  color:'rgba(140,170,220,0.24)' },
    { mode:'multiply', color:'rgba(210,225,245,0.12)' },
  ]},
  silver:    { layers: [
    { mode:'saturation', color:'rgb(220,220,220)', alpha:0.45 },
    { mode:'screen',     color:'rgba(235,240,245,0.10)' },
  ]},
  afterrain: { layers: [
    { mode:'multiply',   color:'rgba(150,165,180,0.20)' },
    { mode:'saturation', color:'rgb(210,210,210)', alpha:0.25 },
  ]},
  bw:        { layers: [
    { mode:'saturation', color:'rgb(128,128,128)' }, // full grayscale
    { mode:'overlay',    color:'rgba(255,255,255,0.08)' }, // kontras biar tegas
  ]},
  sepia:     { layers: [
    { mode:'saturation', color:'rgb(128,128,128)' }, // grayscale dulu
    { mode:'color',      color:'rgba(112,66,20,1)', alpha:0.55 }, // baru tone sepia
  ]},
  midnight:  { layers: [
    { mode:'multiply',   color:'rgba(30,35,60,0.32)' },
    { mode:'saturation', color:'rgb(180,180,180)', alpha:0.25 },
  ]},
};
