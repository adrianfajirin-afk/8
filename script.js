...
function pauseAllMedia(){
  ...
  document.querySelectorAll('.ig-vn-btn.playing').forEach(b=> b.classList.remove('playing'));
...
function buildFeedPost(item){
  ...
  // ==================== TOMBOL PUTAR VN (di samping ikon komentar) ====================
  // Cuma muncul kalau postingan ini emang punya VN ASLI dari tamu. TIDAK
  // pernah autoplay -- baru mulai muter pas tombol ini di-tap manual.
  // Tombolnya bulat + ikon play/pause (mirip pemutar VN di WhatsApp).
  //
  // FIX JEDA LAMA SEBELUM SUARA KELUAR: sebelumnya elemen <audio> buat VN
  // baru dibikin & baru mulai download PAS tombol ditekan -> tamu nunggu
  // proses download itu dulu baru kedengeran suaranya. Sekarang elemen
  // audio-nya dibikin & mulai di-preload (preload="auto") dari SAAT
  // postingan ini dirender ke feed (bukan pas tombol ditekan), jadi pas
  // tombol beneran di-tap datanya udah/lagi di-download di background dan
  // biasanya langsung siap muter. Selama masih beneran nunggu (jaringan
  // lambat / belum selesai buffer), tombolnya nampilin animasi loading
  // (cincin muter) sebagai tanda "lagi nyiapin", bukan diem kayak ngadat.
  if(hasRealVn){
    ...
    vnBtn.className = 'ig-action-btn ig-vn-btn';
    vnBtn.setAttribute('aria-label', 'Putar suara harapan');
    vnBtn.innerHTML = `
      <span class="ig-vn-circle">
    ...
    vnAudioEl.addEventListener('ended', function(){ vnBtn.classList.remove('playing', 'loading'); });
    ...
    vnBtn.addEventListener('click', function(ev){
      ...
      if(vnBtn.classList.contains('playing')){
        ...
        document.querySelectorAll('.ig-vn-btn.playing').forEach(function(b){
          if(b !== vnBtn && b._stopVn) b._stopVn();
        });
