...
// Fallback lewat CDN (raw.githubusercontent.com) — dipakai HANYA kalau api.github.com
// gagal/limit, karena CDN ini kadang menyimpan cache dan bisa telat update.
async function fetchGitHubGalleryViaRaw(){
  const url = `https://raw.githubusercontent.com/${GITHUB_CONFIG.owner}/${GITHUB_CONFIG.repo}/${GITHUB_CONFIG.branch}/${GITHUB_CONFIG.folder}/manifest.json?t=${Date.now()}`;
  try{
    const res = await fetch(url, { cache: 'no-store' });
    if(!res.ok){
      showDebugBanner(`[Raw] Fallback juga gagal: status ${res.status}`);
      return null;
    }
    return mapManifestList(await res.json());
  }catch(e){
    showDebugBanner(`[Raw] Fallback error: ${e && e.name ? e.name : '?'} — ${e && e.message ? e.message : e} — online:${navigator.onLine}`);
    return null;
  }
}
async function fetchGitHubGallery(){
  if(!GITHUB_CONFIG.owner || GITHUB_CONFIG.owner.startsWith('GANTI_')) return null;
  // Baca manifest.json lewat api.github.com (BUKAN raw.githubusercontent.com) supaya SELALU
  // data terbaru — api.github.com tidak pakai CDN cache seperti raw.githubusercontent.com,
  // yang sebelumnya bisa bikin foto baru "hilang" sementara karena data lama masih ke-cache.
  const apiUrl = `https://api.github.com/repos/${GITHUB_CONFIG.owner}/${GITHUB_CONFIG.repo}/contents/${GITHUB_CONFIG.folder}/manifest.json?ref=${GITHUB_CONFIG.branch}`;
  let res;
  try{
    res = await fetch(apiUrl, { cache: 'no-store' });
  }catch(e){
    // Gagal di TAHAP FETCH itu sendiri (request nggak sampai/nggak dapat respons sama sekali).
    // Ini beda dari gagal decode/parse di bawah — kalau errornya muncul di sini terus-menerus
    // walau internet lancar, kemungkinan besar bukan soal jaringan tapi soal header/CORS/preflight.
    showDebugBanner(`[GitHub] FETCH gagal: ${e && e.name ? e.name : '?'} — ${e && e.message ? e.message : e} — online:${navigator.onLine} — url:${apiUrl}`);
    return await fetchGitHubGalleryViaRaw();
  }
  try{
    if(res.ok){
      ...
      let jsonText;
      try{
        jsonText = decodeURIComponent(escape(atob((data.content || '').replace(/\n/g,''))));
      }catch(decodeErr){
        showDebugBanner(`[GitHub] DECODE base64 gagal: ${decodeErr && decodeErr.message ? decodeErr.message : decodeErr}`);
        return await fetchGitHubGalleryViaRaw();
      }
      let parsed;
      try{
        ...
        showDebugBanner(`[GitHub] PARSE json gagal: ${parseErr && parseErr.message ? parseErr.message : parseErr} — isi awal: ${jsonText.slice(0,80)}`);
        return await fetchGitHubGalleryViaRaw();
      }
      const list = mapManifestList(parsed);
      if(list) return list;
      showDebugBanner(`[GitHub] Manifest ke-parse tapi bentuknya bukan array yang diharapkan`);
    } else if(res.status === 404){
      return null; // belum ada momen sama sekali -> wajar untuk situs yang baru mulai dipakai
    } else if(res.status === 403){
      showDebugBanner(`[GitHub] Kena rate limit (403) — pakai jalur cadangan`);
    } else {
      showDebugBanner(`[GitHub] Gagal ambil manifest: status ${res.status}`);
    }
    return await fetchGitHubGalleryViaRaw();
  }catch(e){
    showDebugBanner(`[GitHub] Error tak terduga setelah fetch: ${e && e.name ? e.name : '?'} — ${e && e.message ? e.message : e}`);
    return await fetchGitHubGalleryViaRaw();
  }
}
...
document.getElementById('finalShareBtn').addEventListener('click', async ()=>{
  ...
  if(imageUrl){
    ...
    if(state.audioBlob){
      const audioFilename = uniqueFilename('harapan','webm');
