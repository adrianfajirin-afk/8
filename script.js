...
function buildFeedPost(item){
  ...
  if(item.type === 'video'){
    ...
    // pasang rasio aslinya SEKARANG juga (gak nunggu 'load') biar gak ada
    // celah buat glitch. Kalau belum pernah, biarin dulu pake tebakan
    // default dari CSS, entar di-update di listener 'load' di bawah.
    const cachedRatio = _photoRatioCache.get(item.img);
    if(cachedRatio) media.style.aspectRatio = cachedRatio;
    img.addEventListener('load', ()=>{
      if(img.naturalWidth && img.naturalHeight){
        const ratio = `${img.naturalWidth} / ${img.naturalHeight}`;
        _photoRatioCache.set(item.img, ratio);
...
function renderFeed(items){
  ...
  feedObserver = new IntersectionObserver((entries)=>{
    entries.forEach(entry=>{
      ...
      // Batalin dulu rencana "pause" sebelumnya kalau ada — soalnya kalau
      // post ini balik lagi ke layar sebelum timer-nya kepanggil, berarti
      // tadi cuma kegeser dikit doang, bukan beneran ditinggal.
      if(post._pauseTimer){ clearTimeout(post._pauseTimer); post._pauseTimer = null; }

      if(entry.isIntersecting && ratio >= 0.6){
        // Postingan ini dianggap "lagi ditonton" -> nyalain.
        if(post._videoEl && post._videoEl.paused){ post._videoEl.play().catch(()=>{}); }
        if(post._audioEl && _activeAmbientAudio !== post._audioEl){
          // Reset ke awal cuma pas PERTAMA KALI lagu ini keputer di post
          // ini (foto baru masuk layar) — bukan tiap kali sekadar
          // kegeser dikit terus balik lagi, biar gak "restart mulu" yang
          // kerasa ganggu.
          if(!post._audioStarted){ post._audioEl.currentTime = 0; post._audioStarted = true; }
          playAmbientAudio(post._audioEl);
        }
      } else if(ratio < 0.15){
        // Baru dianggap BENERAN keluar layar kalau udah hampir sepenuhnya
        // ilang (bukan cuma kegeser dikit) — dan dikasih jeda sekejap
        // (debounce) dulu, jadi kalau lagi scroll cepat bolak-balik,
        // musiknya gak keburu mati-nyala berkali-kali dalam sekejap.
        post._pauseTimer = setTimeout(()=>{
          post._pauseTimer = null;
          if(post._videoEl){ post._videoEl.pause(); }
          if(post._audioEl){ pauseAmbientAudio(post._audioEl); }
        }, 220);
      }
      // ratio di antara 0.15 - 0.6 (cuma kegeser dikit): dibiarin apa
      // adanya, gak langsung dimatiin gak juga langsung dinyalain, biar
      // gak "kedap-kedip" tiap scroll dikit.
    });
  }, { root: feedContainer, threshold: [0, 0.15, 0.6, 1] });
