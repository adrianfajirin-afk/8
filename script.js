...
async function renderGallery(){
  ...
  items.forEach(item=>{
    ...
    if(item.type === 'video'){
      // playsinline WAJIB di iOS Safari supaya video "disiapkan" tanpa
      // masuk mode fullscreen. Tanpa ini, video sering tampil hitam
      // polos di galeri sampai user tap play.
      card.innerHTML = `<video src="${item.videoUrl}" muted playsinline preload="metadata" loading="lazy"></video>
      ...
      // Trik thumbnail: begitu metadata siap, geser sedikit ke depan
      // (bukan persis 0 detik) supaya Safari mau render & menampilkan
      // frame itu sebagai gambar diam di kartu galeri.
      thumbVideo.addEventListener('loadedmetadata', ()=>{
        try{ thumbVideo.currentTime = Math.min(0.1, thumbVideo.duration || 0.1); }catch(e){}
      }, { once:true });
    } else {
