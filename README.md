...
## Tentang manifest.json (data galeri)
Setiap kali tamu tekan "Bagikan Momen", selain foto (dan suara kalau ada),
sistem juga otomatis mencatat 1 baris ke `uploads/manifest.json` di repo
GitHub — isinya nama pengirim, link foto, link suara, dan waktu kirim.
File ini yang dibaca ulang setiap kali galeri dibuka, supaya nama & suara
tetap muncul buat semua orang (bukan cuma di HP pengirimnya), bahkan
setelah refresh atau dibuka di HP lain.

Kalau baru pertama kali dipakai, file `uploads/manifest.json` memang belum
ada — itu normal, akan otomatis dibuat begitu ada 1 momen pertama yang
berhasil dibagikan.
